export const faqs = [
  ...
  },
  {
    ...
    text:
      "Redux works by maintaining a single source of truth, called the store, which holds the entire state of the application. Actions are dispatched to modify this state, and reducers define how the state should be updated based on these actions."
  },
  {
  ...
  },
  {
    ...
    text:
      "You can use the 'react-redux' library to connect Redux to a React application. It provides a 'Provider' component to wrap your app with, and a 'connect' function to connect individual components to the Redux store."
  },
  {
  ...
  },
  {
    ...
    text:
      "Yes, Redux can be used in both client-side and server-side environments. This can be useful for isomorphic/universal applications, where the same code runs on both the server and the client."
  },
  {
...

  {
    ...
    text:
      "You can use middleware like 'redux-thunk' or 'redux-saga' to handle asynchronous operations in Redux by dispatching actions that represent the different states of the operation (e.g., request, success, failure)."
  },
  {
    title: "What is the difference between mapStateToProps and mapDispatchToProps?",
    text:
      "mapStateToProps is used to map parts of the Redux store state to props in a React component, while mapDispatchToProps is used to map action creators to props, allowing components to dispatch actions."
  }
];
